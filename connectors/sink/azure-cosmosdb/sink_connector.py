...
class SinkConnectorCosmosDB:
    ...
    def _create_database_if_not_exists(self):
        # Creating database
        try:
            self.cosmos_db = self.cosmos_client.create_database(id=self.cosmosdb_database_name)
            print('Database with id \'{0}\' created'.format(self.cosmosdb_database_name))
        except errors.CosmosResourceExistsError:
            print('A database with id \'{0}\' already exists'.format(self.cosmosdb_database_name))
            self.cosmos_db = self.cosmos_client.get_database_client(database=self.cosmosdb_database_name)
    ...
    def _create_container_if_not_exists(self):
        ...
        try:
            ...
            self.cosmos_db_container = self.cosmos_db.get_container_client(container_name)
        except Exception as e:
            print('Error creating container: ', e)

    def put(self, data):
        container_link = f"dbs/{self.cosmosdb_database_name}/colls/{self.cosmosdb_container_name}"
        self.cosmos_db_container.upsert_item(data)
